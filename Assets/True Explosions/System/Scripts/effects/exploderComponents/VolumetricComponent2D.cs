...
public class VolumetricComponent2D : ExploderComponent{
    ...
    private void makeStep()
    {
        ...
        for (int i = 0; i < curCount; i++)
        {
            // 修正: ParticleSystem.Particle.size ではなく startSize を使用
    ...
    private void resetColors()
    {
        ...
        for (int i = 0; i < curCount; i++)
        {
            // 修正: ParticleSystem.Particle.color ではなく startColor を使用
    ...
    protected void moveParticle(int id, float distance)
    {
        ...
        int hitCount = Physics2D.RaycastNonAlloc(testRay.origin, testRay.direction, hits, distance);
